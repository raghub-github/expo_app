-- Single source of truth for store open/closed. UI must read live_status only.
-- Store is OPEN only when: is_active AND is_available AND is_accepting_orders AND operational_status = 'OPEN'.

-- 1. View: computed live_status (no client-side guessing)
CREATE OR REPLACE VIEW public.store_live_status AS
SELECT
  id,
  store_id,
  CASE
    WHEN is_active = true
     ...
     AND UPPER(COALESCE(operational_status::text, '')) = 'OPEN'
    THEN 'OPEN'
    ELSE 'CLOSED'
  END AS live_status,
  updated_at
...
COMMENT ON VIEW public.store_live_status IS
  'Computed store status. OPEN only when is_active, is_available, is_accepting_orders true and operational_status = OPEN.';

-- 2. Add is_available to get_nearby_merchant_stores so backend can compute same live_status
...
COMMENT ON FUNCTION public.get_nearby_merchant_stores(double precision, double precision, double precision, integer, boolean) IS
  'Stores within 15km. Includes is_available for live_status. OPEN only when is_active, is_available, is_accepting_orders true and operational_status=OPEN.';
