-- Veg mode + ultra-strict location engine for food delivery.
-- Rules: (1) radius_km capped at 15 for listing. (2) veg_mode = true → only pure veg or stores with veg menu items.
-- (3) Reject corrupted coordinates: latitude -90..90, longitude -180..180.
...
COMMENT ON FUNCTION public.get_nearby_merchant_stores(double precision, double precision, double precision, integer, boolean) IS
  'Stores within radius_km (max 15) of (user_lat, user_lng). Haversine. veg_mode=true → only pure veg or stores with veg menu items. Rejects invalid coords.';
