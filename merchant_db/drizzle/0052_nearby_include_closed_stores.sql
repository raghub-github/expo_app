-- Include closed restaurants in nearby listing; availability driven by is_accepting_orders + operational_status.
-- Listing: is_active = true, approval_status = 'APPROVED'. Open/closed from DB, no filter by is_accepting_orders.
-- Frontend: "Open Now" filter → only is_accepting_orders = true AND operational_status = 'OPEN'.
...
COMMENT ON FUNCTION public.get_nearby_merchant_stores(double precision, double precision, double precision, integer, boolean) IS
  'Stores within 15km. Includes open and closed; approval_status=APPROVED. is_accepting_orders and operational_status for frontend Open Now filter.';
