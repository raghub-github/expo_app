...
const PreviewPage = ({ step1, step2, documents, storeSetup, menuData, parentInfo, onBack, onContinueToPlans, actionLoading = false }: PreviewPageProps) => {
  ...
  return (
    <div className="h-screen w-full bg-gradient-to-br from-slate-50 to-white overflow-hidden">
      ...
      <div className="h-[calc(100vh-5rem)] w-full overflow-y-auto scrollbar-hide pb-24">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-8">
              ...
              {/* Bank Details */}
              <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
                <div className="px-6 py-4 bg-gradient-to-r from-sky-50 to-indigo-50 border-b border-slate-200">
                  <h2 className="text-lg font-semibold text-slate-900 flex items-center gap-2">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-10 bg-white border-t border-slate-200 px-4 py-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)]">
        <div className="max-w-7xl mx-auto flex items-center justify-end gap-3">
          <button
            ...
            className="px-5 py-2.5 border border-slate-300 text-slate-700 rounded-lg hover:bg-slate-50 transition-colors flex items-center gap-2 font-medium disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {actionLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : <ChevronLeft className="w-4 h-4" />}
            Back
          </button>
          <button
            onClick={onContinueToPlans}
            disabled={actionLoading}
            className="px-8 py-2.5 bg-gradient-to-r from-indigo-600 to-purple-600 text-white rounded-lg hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 font-semibold flex items-center gap-2 shadow-md hover:shadow-lg disabled:opacity-60 disabled:cursor-not-allowed"
          >
            {actionLoading ? (
            ...
            {actionLoading ? 'Loading...' : 'Continue to plans'}
          </button>
        </div>
      </div>
