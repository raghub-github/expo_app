...
export async function POST(req: NextRequest) {
  ...
  if (action === 'send') {
    ...
    const terminalOtpMode = isTerminalOtpEnabled()
    const devOtp = terminalOtpMode ? generateDevOtp() : undefined
    console.log('[api/auth/otp] sending via MSG91', { mobile: `${mobile.slice(0, 4)}****${mobile.slice(-2)}` })
    if (devOtp) {
      console.log('[api/auth/otp] DEV OTP', { mobile, otp: devOtp })
      // Keep OTP server-side for local verification in terminal OTP mode.
      devOtpStore.set(mobile, { otp: devOtp, expiresAt: Date.now() + 10 * 60 * 1000 })
    }
    const sent = await msg91SendOtp(mobile, devOtp)
    if (!sent.ok) {
      if (devOtp) devOtpStore.delete(mobile)
    ...
    console.log('[api/auth/otp] MSG91 send accepted')
