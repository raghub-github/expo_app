...
export async function pickCatalogPhoto(
  ...
  // Cast avoids the enum shape drifting between expo-image-picker minor
  // versions (MediaTypeOptions vs MediaType[]). `ImagePicker` here is the
  // dynamically-imported module (value), so we can't use it as a type — use
  // `Parameters<...>[0]` off one of its picker calls to stay version-agnostic.
  const pickerOpts = {
    mediaTypes:
