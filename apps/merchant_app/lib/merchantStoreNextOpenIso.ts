 ...
 * Merchant app: close-for-today + countdown parity with dashboard / Partner Site.
 */

import type { DaySlots, OperatingHours } from "@/services/outletApi";
...
/** Nested operating-hours API payload → flat row shape expected by `getNextOpenIso` (DB / schedule engine). */
export function operatingHoursToFlatRow(h: OperatingHours): Record<string, unknown> {
  const out: Record<string, unknown> = {
    is_24_hours: h.is_24_hours === true,
    same_for_all_days: h.same_for_all_days === true,
    closed_days: Array.isArray(h.closed_days) ? h.closed_days : [],
  };
  for (const day of DAY_NAMES) {
    const slots = h[day] as DaySlots | undefined;
    out[`${day}_open`] = slots?.open === true;
    out[`${day}_slot1_start`] = slots?.slot1_start ?? null;
    out[`${day}_slot1_end`] = slots?.slot1_end ?? null;
    out[`${day}_slot2_start`] = slots?.slot2_start ?? null;
    out[`${day}_slot2_end`] = slots?.slot2_end ?? null;
  }
  return out;
}
...
/** First opening on a future IST calendar day only — for "Close for today". */
