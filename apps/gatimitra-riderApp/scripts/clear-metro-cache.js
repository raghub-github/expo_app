/**
 * Windows-safe Metro cache wipe for gatimitra-riderApp.
 * Renames the cache root when files are locked, then deletes the old folder best-effort.
 */
const fs = require('fs');
const os = require('os');
...
function clearMetroCacheRoot() {
  ...
  try {
    ...
    console.warn(
      `[clear-metro-cache] Could not clear cache (stop other Metro on port 8081): ${err.message}`,
    );
    process.exitCode = 1;
  }
}
