...
export function segmentFontFamily(kind: TypographySegmentKind, bold: boolean): string {
  if (kind === "numeric") {
    return bold ? RiderFonts.poppinsBold : RiderFonts.poppinsSemiBold;
  }
  return bold ? RiderFonts.loraBold : RiderFonts.loraRegular;
}

const BRAND_FACES = new Set<string>([
  ...Object.values(RiderFonts),
  "Lora_600SemiBold",
]);

/** Keep mono / icon / third-party faces untouched by AppText. */
