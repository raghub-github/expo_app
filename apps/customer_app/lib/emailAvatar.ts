/**
 * Profile photo from verified email — Gravatar, GitHub (via unavatar), stored URL.
 */
...
function md5Hex(input: string): string {
  const utf8 = unescape(encodeURIComponent(input.trim().toLowerCase()));
...
/** Direct unavatar image URL — resolves Gravatar/GitHub for the email. */
export function resolveClientEmailAvatarUrl(email: string, size = 256): string {
  const normalized = email.trim().toLowerCase();
  return `https://unavatar.io/${encodeURIComponent(normalized)}?size=${size}`;
}

export function resolveClientGithubAvatarUrl(email: string, size = 256): string {
  const normalized = email.trim().toLowerCase();
  return `https://unavatar.io/github/${encodeURIComponent(normalized)}?size=${size}`;
}

/** Ordered candidates — try each until one loads in the Image component. */
export function buildEmailAvatarCandidates(
  ...
  const push = (url: string | null | undefined) => {
    ...
    if (!trimmed || isGenericProfileImageUrl(trimmed)) return;
    if (!out.includes(trimmed)) out.push(trimmed);
  };

  push(storedUrl);
  push(resolveClientEmailAvatarUrl(normalized, size));
  push(resolveClientGithubAvatarUrl(normalized, size));
  push(getGravatarUrl(normalized, size));

  return out;
}

export function pickVerifiedEmailAvatarUrl(
  email: string | null | undefined,
  storedUrl?: string | null,
  size = 256,
): string | null {
  return buildEmailAvatarCandidates(email, storedUrl, size)[0] ?? null;
}
