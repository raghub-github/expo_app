...
export function resolveRideBookingPayableTotal(order: {
  ...
  if (Number.isFinite(fromMeta) && fromMeta >= 0 && meta != null && meta.quotedGrandTotal != null && meta.quotedGrandTotal !== "") {
    const tip = Math.max(0, Number(order.tipAmount ?? meta?.customerTipAmount ?? 0));
    return Math.max(0, Math.round((fromMeta - tip) * 100) / 100);
  }
  const snap =
  ...
  if (Number.isFinite(fromSnap) && fromSnap >= 0 && snap != null && snap.final_amount != null && snap.final_amount !== "") {
    const tip = Math.max(0, Number(order.tipAmount ?? snap?.tip_amount ?? 0));
    return Math.max(0, Math.round((fromSnap - tip) * 100) / 100);
  }
