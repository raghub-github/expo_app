...
export default function OtpScreen() {
  ...
  const params = useLocalSearchParams<{ requestId: string; phoneE164: string; otp?: string }>();
  const requestId = params.requestId ?? "";
  const phoneE164 = params.phoneE164 ?? "";
  const initialOtp = params.otp ?? "";
  ...
  useEffect(() => {
    if (initialOtp && initialOtp.length <= OTP_LENGTH) setOtp(initialOtp);
  }, [initialOtp]);
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      try {
        ...
        // Only go to home when profile is explicitly completed (user exists in DB and finished onboarding)
        ...
        <View style={styles.card}>
          ...
          <View style={styles.cardInner}>
            ...
            <TouchableOpacity
              ...
              disabled={loading}
            >
              <Ionicons name="arrow-back" size={18} color={LINK_GREEN} />
