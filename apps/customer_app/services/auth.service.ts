  ...
  async sendOtp(payload: SendOtpPayload): Promise<void> {
    ...
    if (__DEV__) {
      if (error) {
        ...
        // eslint-disable-next-line no-console
        console.log(
          "[CustomerAuth] sendOtp: Supabase OK (session stays null until verify — normal). If no SMS: set Auth → Hooks → Send SMS to your public API /v1/auth/supabase-send-sms with MSG91_* on the server.",
        );
      }
    }
