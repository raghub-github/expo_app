...
export function RideTripShareSheet({ visible, orderId, onClose }: RideTripShareSheetProps) {
    ...
    async (channel: ShareChannel) => {
      ...
      try {
        if (channel === "copy") {
          ...
          Alert.alert("Link copied", "Live trip link copied to clipboard.");
          return;
        }
        if (channel === "whatsapp") {
        ...
        if (channel === "telegram") {
          await Linking.openURL(`https://t.me/share/url?url=${encodedUrl}&text=${encodeURIComponent("Track my GatiMitra trip")}`);
          return;
        }
        await Share.share({ message: shareMessage, url: shareUrl, title: "GatiMitra Live Trip" });
      } catch {
  ...
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={() => onCloseRef.current()}>
...
const styles = StyleSheet.create({
  overlay: { flex: 1, justifyContent: "flex-end", zIndex: 100 },
