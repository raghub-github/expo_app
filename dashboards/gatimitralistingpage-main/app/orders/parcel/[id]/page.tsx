...
import { mockParcelOrders } from '@/components/orders/ParcelDashboard';

export default function ParcelOrderDetailsPage() {
  const router = useRouter();
  const params = useParams();
  const orderId = params?.id as string;
  const order = mockParcelOrders.find(o => o.orderId === orderId);

  useEffect(() => {
    if (!order) router.replace('/orders/parcel');
  }, [order, router]);
  ...
  return (
    <div className="max-w-2xl mx-auto p-8 bg-white rounded shadow mt-8">
      <h2 className="text-2xl font-bold mb-4">Parcel Order Details</h2>
