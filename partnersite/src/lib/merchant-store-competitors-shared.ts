...
export type LocalityInsight = {
  ...
  /** Human locality label (area name), not raw pincode. */
...
export type MerchantMarketInsights = {
  ...
  /** % of your 90d customers who also ordered from ≥1 peer in this scope. */
...
/** Short area name from full address (not pincode). */
...
/** Unified competitor + own-store row for the market insights list. */
export type CompetitorLeaderboardRow = {
  ...
  rank_delta: number | null;
  is_own: boolean;
  /** e.g. "1-1", "1-2" — primary rank group + affinity order within group. */
  display_rank: string;
};

/**
 * Merge own store into competitors, sort by affinity (desc).
 * Own store wins affinity ties and appears first when it leads.
 * Display ranks: 1-1, 1-2, 1-3… (affinity order).
 */
