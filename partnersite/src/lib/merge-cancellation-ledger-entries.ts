 ...
 * Keep in sync with backend/src/lib/merge-cancellation-ledger-entries.ts
 */
...
/** Union core / food / formatted ids so rows for the same order share one bucket key. */
...
function isCancellationCompensationCredit(entry: MergeableLedgerEntry): boolean {
  ...
  // Any ORDER_ADJUSTMENT credit on a cancelled order (paired with info row later).
  return true;
}
...
/** Original order value for strikethrough — never the compensation credit itself. */
