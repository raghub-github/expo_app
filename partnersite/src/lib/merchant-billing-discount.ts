...
/** True when discount is merchant store precision (not platform / Boost / BOGO). */
export function isCartSurfaceDiscountRow(row: Record<string, unknown>): boolean {
  return isMerchantPrecisionDiscountRow(row);
}

/**
 * Merchant store precision offer only. Platform checkout offers / coupons never match.
 */
...
/** Merchant precision only — platform offers/coupons excluded. */
