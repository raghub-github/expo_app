 ...
 * Fetched via partnersite API proxy to backend /v1/app-assets/merchant.
 */
...
/** Short keys in API `assets` map (prefix `merchant.` stripped). */
...
/** Drop cached URLs after super-admin uploads (partnersite refetches on next load). */
...
export async function getMerchantAppAssetUrlAsync(key: string): Promise<string | null> {
  const assets = await loadMerchantAppAssets();
  return resolveMerchantAssetUrl(assets[key]);
}
