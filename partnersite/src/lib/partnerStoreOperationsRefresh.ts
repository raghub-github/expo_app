...
export type PartnerStoreOperationsRefreshDetail = {
  storeId: string;
  forceClosed?: boolean;
  isDelisted?: boolean;
};

export function emitPartnerStoreOperationsRefresh(
  storeId: string,
  extra?: { forceClosed?: boolean; isDelisted?: boolean },
): void {
  if (typeof window === 'undefined' || !storeId) return;
  window.dispatchEvent(
    new CustomEvent<PartnerStoreOperationsRefreshDetail>(PARTNER_STORE_OPERATIONS_REFRESH_EVENT, {
      detail: { storeId, ...extra },
    })
  );
}
