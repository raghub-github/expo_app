...
const KEY_PREFIX = "gm_local_store_status_engine_";
const TICK_MS = 5_000;

type EngineStore = {
  state: StoreStatusEngineState;
  hydrate: (storeKey: string) => void;
  /** Align local state + persistence with GET /api/store-operations (dashboard / other tabs / DB). */
...
export const useLocalStoreStatusEngineStore = create<EngineStore>((set, get) => ({
  ...
  syncFromStoreOperations: ({ operationalOpen, manualCloseUntil, manualCloseReason }) => {
    ...
    set((prev) => {
      const patch: Partial<StoreStatusEngineState> = {
        store_status: operationalOpen ? "ONLINE" : "OFFLINE",
        manual_close_until: manualCloseUntil,
        manual_close_reason: manualCloseReason,
        is_manual_override: false,
        manual_override_at: null,
        schedule_end_prompt_expires_at: null,
        // Server already applied schedule + operational_status; do not let TICK reopen while DB says closed.
  ...
  manualOn: () => {
    ...
    set((prev) => {
      ...
      if (effects.some((e) => e.type === "persist") && currentKey) persist(currentKey, state);
      runEffects(effects, { openModal: get().openScheduleEndModal });
      // No platform toast dependency here; callers can show UI_STRINGS.manualOnBeforeSchedule if desired.
      return { state };
    });
  },
  ...
  manualOff: () => {
    const now = new Date();
    set((prev) => {
      const { state, effects } = reduceStoreStatusEngine(prev.state, { type: "MANUAL_OFF", now });
      if (effects.some((e) => e.type === "persist") && currentKey) persist(currentKey, state);
      runEffects(effects, { openModal: get().openScheduleEndModal });
      return { state };
    });
  ...
  tempClose: (untilIso, reason) => {
    const now = new Date();
    set((prev) => {
      const { state, effects } = reduceStoreStatusEngine(prev.state, { type: "TEMP_CLOSE", now, manual_close_until: untilIso, manual_close_reason: reason ?? null });
      if (effects.some((e) => e.type === "persist") && currentKey) persist(currentKey, state);
      return { state };
    });
  },
