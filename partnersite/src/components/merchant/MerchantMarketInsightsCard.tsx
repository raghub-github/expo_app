...
export function MerchantMarketInsightsCard({
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/merchant/market/insights?${q}`, { credentials: "include" });
      ...
      {error && !data ? (
        ...
        <div className="divide-y divide-slate-200/70 flex-1">
          {Array.from({ length: 5 }, (_, i) => (
            <div key={i} className="grid grid-cols-1 sm:grid-cols-12 gap-2 sm:gap-3 items-center py-3.5">
              <div className="sm:col-span-3 h-4 w-28 animate-pulse rounded bg-slate-200/80" />
              ...
              <div className="sm:col-span-4 flex justify-end gap-2">
                <div className="h-5 w-14 animate-pulse rounded bg-slate-200/80" />
                <div className="h-5 w-12 animate-pulse rounded-full bg-slate-200/80" />
              </div>
            </div>
          ))}
        </div>
