...
interface Step4DocumentsProps {
  ...
  /** When provided, section is controlled by parent (e.g. for Save & Continue ΓåÆ next section). */
...
const Step4Documents: React.FC<Step4DocumentsProps> = ({
  ...
  const documentFormatValidators = {
    ...
    // Drug licence: different states have different formats ΓÇô basic non-empty + min length check
    drug: (v: string) =>
  ...
  const renderAadhaarSection = () => (
    <div className="space-y-4">
      <div className="rounded-xl border border-indigo-100 bg-indigo-50/70 px-4 py-3">
        ...
        <p className="mt-1 text-[11px] sm:text-xs text-indigo-700">
          Identity verification. Images are optionalΓÇönumber and name are sufficient.
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div>
  ...
  const renderGstSection = () => (
    <div className="space-y-2">
        ...
        <>
          ...
          <div className="rounded-lg border border-amber-200 bg-amber-50/80 px-3.5 py-2">
            <p className="text-[11px] sm:text-xs font-semibold text-amber-800">
              FSSAI Certificate (Mandatory) ΓÇô FSSAI license is mandatory for restaurant as per food safety regulations.
            </p>
          </div>
      ...
      {/* Additional licences shared across store types ΓÇô shown as a next (skippable) step */}
      {!showOtherLicences &&
      ...
      !form.other_expiry_date ? (
        ...
        <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50/80 px-4 py-3 space-y-3">
          ...
          {/* Udyam Registration is valid for lifetime ΓÇô no expiry field */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-700 mb-1">
                Other Document Type
