-- Rollback 0473_store_requires_visible_menu.sql
-- Restores nearby RPC without has_customer_visible_menu filter (0235 semantics).

DROP TRIGGER IF EXISTS merchant_menu_categories_refresh_customer_visible ON public.merchant_menu_categories;
DROP FUNCTION IF EXISTS public.trg_refresh_store_customer_visible_menu_from_category();
DROP TRIGGER IF EXISTS merchant_menu_items_refresh_customer_visible ON public.merchant_menu_items;
DROP FUNCTION IF EXISTS public.trg_refresh_store_customer_visible_menu();
DROP FUNCTION IF EXISTS public.refresh_store_customer_visible_menu(bigint);
DROP FUNCTION IF EXISTS public.store_has_customer_visible_menu(bigint);

DROP INDEX IF EXISTS public.merchant_stores_has_customer_visible_menu_idx;
ALTER TABLE public.merchant_stores DROP COLUMN IF EXISTS has_customer_visible_menu;

-- Revert nearby RPC to pre-0473 (include closed, no menu-visibility gate).
-- Source: 0235_get_nearby_merchant_stores_include_closed.sql
