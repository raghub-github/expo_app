...
-- Only migrate if old table exists and has the required columns
DO $$
DECLARE
...
BEGIN
  ...
  ELSE
    v_column_exists := FALSE;
  END IF;
  
  -- Only migrate if both table and column exist
  ...
  ELSE
    -- If table doesn't exist or doesn't have day_of_week column, skip migration
    RAISE NOTICE 'Skipping data migration: merchant_store_operating_hours table does not exist or does not have day_of_week column';
  END IF;
END $$;
