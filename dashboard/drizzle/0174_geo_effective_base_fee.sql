-- Effective base_fee per service: walk chain from current node up to state (nearest rule wins).
-- Used by geo tree / search UI to show ₹ amounts next to coverage toggles.
...
CREATE OR REPLACE FUNCTION geo_effective_base_fee(
  p_level geo_pricing_level,
  p_id uuid,
  p_service geo_service,
  p_rule_type text DEFAULT 'base_fee'
) RETURNS numeric
LANGUAGE plpgsql
STABLE
AS $$
...
BEGIN
  ...
  LOOP
    SELECT g.value_numeric INTO v
    FROM geo_service_pricing_rules g
    WHERE g.level = rec.step_level
      AND g.ref_id = rec.step_id
      AND g.service = p_service
      AND g.rule_type = p_rule_type
      AND g.is_active = true
    ORDER BY g.priority DESC NULLS LAST, g.created_at DESC
    LIMIT 1;
