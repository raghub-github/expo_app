...
-- Mirror: backend/drizzle/0494_platform_offer_coupon_code.sql
...
-- Backfill from offer name (strip non-alphanumeric) or GM{id} fallback.
...
-- Normalize any existing values to uppercase allowed charset.
UPDATE public.billing_platform_offers
...
-- Resolve case-insensitive collisions by appending -{id} to later rows.
WITH ranked AS (
  SELECT
    id,
