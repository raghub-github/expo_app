...
-- Ensure onboarding bank/UPI columns exist so auto-verify + manual save
-- work on partnersite + AM without column-missing errors.
-- Mirror of backend/drizzle/0452_merchant_store_bank_accounts_onboarding_verify.sql
-- ============================================================================
...
BEGIN
  ...
  -- Payout channel + proof fields used by register-store / AM onboarding
  ...
  -- Verification columns (may already exist from 0010)
  ALTER TABLE public.merchant_store_bank_accounts
  ...
  -- Soften NOT NULL for UPI-only rows (placeholders still allowed)
  BEGIN
    ALTER TABLE public.merchant_store_bank_accounts
      ALTER COLUMN account_number DROP NOT NULL;
  EXCEPTION WHEN others THEN
    NULL; -- already nullable or constrained differently
  END;
  BEGIN
    ALTER TABLE public.merchant_store_bank_accounts
