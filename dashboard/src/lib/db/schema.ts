...
export type ActionType =
  ...
  | "RIDER_SUBSCRIPTION_DUES_CLEARED"
  | "REQUEST_CREATE"
  | "REQUEST_APPROVE"
  | "REQUEST_REJECT"
...
const ridersTable = pgTable(
  "riders",
  {
    ...
    /** Current working location enrichment (may change over time). */
...
/**
 * Append-only working location history.
 * riders.* holds the current working location; this table keeps prior snapshots.
 */
...
export const riderPaymentMethods = pgTable(
  "rider_payment_methods",
  {
    ...
    crossCheckStatus: text("cross_check_status"),
    crossCheckMessages: jsonb("cross_check_messages").$type<string[] | null>(),
