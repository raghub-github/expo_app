...
export function merchantBaseUnitForItem(
  ...
  customerUnitFallback: number,
  commissionPercent?: number
): number {
  const snap = snapshots.find((s) => s.orderItemId === orderItemId);
  if (snap && snap.merchantBasePerUnit > 0) return snap.merchantBasePerUnit;
  if (
    commissionPercent != null &&
    Number.isFinite(commissionPercent) &&
    commissionPercent >= 0 &&
    commissionPercent < 100 &&
    customerUnitFallback > 0
  ) {
    return round2((customerUnitFallback * (100 - commissionPercent)) / 100);
  }
  return customerUnitFallback;
}
