...
function resolveLogoUrl(raw: string | null | undefined): string | null {
  ...
  if (t.startsWith("/v1/attachments/proxy")) {
...
export async function loadMerchantMarketInsights(
  ...
  if (competitors.length < cap) {
    ...
    const peers = await loadAreaPeerCompetitors(sql, storePk, store, scope, cap - competitors.length, seen);
  ...
  const areaOrderTotal =
    yourOrders + competitors.reduce((sum, c) => sum + (orderByStoreId.get(c.competitor_store_id) ?? 0), 0);
