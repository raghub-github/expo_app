...
export async function lookupStoreServiceType(storeId: number, sql: Sql = getSql()): Promise<string> {
  try {
    const rows = await sql`
...
export async function lookupOrderContext(orderCoreId: number, sql: Sql = getSql()) {
  const rows = await sql`
