...
/** Sentinel id for delivery fee row in order items / refund UI */
export const DELIVERY_FEE_ITEM_ID = -1;
import {
  extractItemsArray,
  mapCoreDbItemsToRaw,
  normalizeOrderItems,
...
export function itemsFromBillingSnapshot(core: Record<string, unknown>): unknown {
...
export async function loadOrderItemsForCorePk(
  db: SupabaseClient,
  corePk: number
): Promise<{
  ...
  if (!core) {
    return { core: null, food: null, items: [] };
  }
  ...
  const keys = collectCoreItemOrderKeys(core as Record<string, unknown>, food as Record<string, unknown> | null);
  ...
  return {
    ...
    food: (food as Record<string, unknown>) ?? null,
    items,
  };
}
