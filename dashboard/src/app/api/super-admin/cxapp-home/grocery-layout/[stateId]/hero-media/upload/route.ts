/**
 * POST — upload image or MP4 for grocery grid-first hero carousel.
 */
...
export async function POST(request: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    const r2Key = `cxapp-home/grocery-grid-first-hero/${stateId}/${stamp}.${safeExt}`;
    ...
    const items = await saveStateGroceryGridFirstHeroMedia(stateId, [...existing, item]);
    return NextResponse.json({ stateId, item, items }, { status: 201 });
  } catch (e) {
    console.error("[POST grocery hero-media/upload]", e);
