/**
 * GET / PUT — grid-first hero media list for a state.
 */
...
export async function GET(_request: NextRequest, ctx: RouteCtx) {
  ...
  try {
    const items = await getStateGridFirstHeroMedia(stateId);
...
export async function PUT(request: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    const saved = await saveStateGridFirstHeroMedia(stateId, items);
...
export async function DELETE(request: NextRequest, ctx: RouteCtx) {
  ...
  try {
    ...
    const items = await saveStateGridFirstHeroMedia(stateId, remaining);
