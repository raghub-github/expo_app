...
const postBody = z.object({
  level: levelSchema,
  refId: z.string().uuid(),
  platform_offer_id: z.number().int().positive(),
});
...
export async function GET(req: NextRequest) {
  ...
  try {
    const bindings = await listGeoPlatformOfferBindingsForNode(
...
export async function POST(req: NextRequest) {
  ...
  try {
    const binding = await insertGeoPlatformOfferBinding({
