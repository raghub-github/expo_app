...
import { getGeoPricingContext } from "@/lib/db/operations/geo-admin";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const ctx = await getGeoPricingContext(level.data, refId);
    return NextResponse.json(ctx);
  } catch (e) {
    const msg = e instanceof Error ? e.message : "Failed to load pricing context";
