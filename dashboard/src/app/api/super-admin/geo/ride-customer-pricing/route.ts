...
import {
  getEffectiveRideCustomerPricing,
  insertRideCustomerPricing,
...
export async function GET(req: NextRequest) {
  ...
  try {
    if (effective) {
      const result = await getEffectiveRideCustomerPricing({
        level: level.data,
        refId,
    ...
    const slabs = await listRideCustomerPricing({
      level: level.data,
      refId,
      vehicleType: vehicleType.data,
    });
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const slab = await insertRideCustomerPricing({
      level: parsed.data.level,
      refId: parsed.data.refId,
      vehicleType: parsed.data.vehicleType as RideVehiclePricingType,
