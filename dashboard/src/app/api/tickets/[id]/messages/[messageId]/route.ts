...
export async function PATCH(
  ...
  try {
    ...
    if (!userIsSuperAdmin && diffMinutes > 5) {
    ...
    await sqlClient`
      ...
      SET message_text = ${messageText}, updated_at = NOW()
      WHERE id = ${msgId} AND ticket_id = ${ticketId}
    `;
    ...
    console.error("[PATCH /api/tickets/[id]/messages/[messageId]] Error:", error);
    return NextResponse.json(
      {
        success: false,
...
export async function DELETE(
  ...
  try {
    ...
    if (!userIsSuperAdmin && diffMinutes > 5) {
      return NextResponse.json({ success: false, error: "Message can only be deleted within 5 minutes" }, { status: 400 });
    }
