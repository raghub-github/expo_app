...
export async function GET(
  ...
  try {
    ...
    const storePayload = {
      ...
      deleted_at: store.deleted_at ? new Date(store.deleted_at).toISOString() : null,
      delisted_at: store.delisted_at ? new Date(store.delisted_at).toISOString() : null,
    ...
    const safeDocuments = legalDocsRestricted
      ? redactStoreLegalDocuments(documents)
      : documents;
    const safeAgreement = legalDocsRestricted
      ? redactStoreAgreement(agreementAcceptance)
      : agreementAcceptance;
    const safeBankAccounts = legalDocsRestricted
    ...
    return NextResponse.json({
      ...
      operatingHours,
      agreementAcceptance: safeAgreement,
