...
function storedDocumentProxyUrl(r2Key: string): string {
  return (
    toStoredDocumentUrl(r2Key) ?? `/api/attachments/proxy?key=${encodeURIComponent(r2Key)}`
  );
}
...
/** POST multipart — merchant licence renewal upload from control dashboard */
export async function POST(
  ...
  try {
    ...
    const side =
      sideRaw != null && String(sideRaw).trim() !== ""
        ? String(sideRaw).trim().toLowerCase()
        : "front";
    ...
    return NextResponse.json({
      ...
      is_verified: false,
      message:
        "Document uploaded. Gatimitra team will verify it before the store can go online.",
    });
  } catch (err) {
