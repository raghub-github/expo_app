...
function formatStatus(order: ParcelOrder): string {
  const raw = order.currentStatus?.trim() || order.status?.trim() || "—";
  return formatRiderOrderStatusDisplayLabel(raw, "parcel");
}

function formatVehicle(order: ParcelOrder): string {
  const raw =
    order.vehicleCategory?.trim() || order.vehicleTypeRequired?.trim() || "—";
  return raw.replace(/_/g, " ").replace(/\b\w/g, (c) => c.toUpperCase());
}

function formatReceiver(order: ParcelOrder): { name: string; phone: string | null } {
  const name =
    order.receiverName?.trim() || order.customerName?.trim() || "—";
  const phone =
    order.receiverMobile?.trim() || order.customerMobile?.trim() || null;
  return { name, phone };
}

function formatPackage(order: ParcelOrder): string {
  const parts: string[] = [];
  if (order.weightKg != null) parts.push(`${order.weightKg} kg`);
  if (order.lengthCm != null && order.widthCm != null && order.heightCm != null) {
    parts.push(`${order.lengthCm}×${order.widthCm}×${order.heightCm} cm`);
  }
...
function displayOrderId(order: ParcelOrder): string {
  const formatted = order.formattedOrderId?.trim();
  if (formatted) return formatted.replace(/^#/, "");
  return `GMC${String(order.id).padStart(6, "0")}`;
}

export type ParcelOrdersFilters = {
  page: number;
  limit: number;
  status: string;
  dateFrom: string;
...
export async function fetchParcelOrders(
  params: ParcelOrdersFilters,
  signal?: AbortSignal
): Promise<{
  orders: ParcelOrder[];
  total: number;
  ...
  const res = await fetch(`/api/orders/parcel?${qs.toString()}`, {
    credentials: "include",
    signal,
  });
...
export default function ParcelOrdersClient() {
  ...
  const urlSearchType = normalizeParcelSearchType(searchParams.get("searchType"));
  ...
  useEffect(() => {
    ...
    router.replace(qs ? `/dashboard/orders/parcel?${qs}` : "/dashboard/orders/parcel", {
      scroll: false,
    });
  ...
  const filtersForQuery: ParcelOrdersFilters = useMemo(
    () => ({
      page: urlPage,
      limit,
      status: urlStatus,
  ...
  const queryKey = useMemo(
    () => ["parcel-orders", filtersForQuery] as const,
    [filtersForQuery]
  );
  ...
  const snapshotKey = useMemo(
    () =>
      `dashboard_snapshot:parcel_orders:/dashboard/orders/parcel:${JSON.stringify(filtersForQuery)}`,
    [filtersForQuery]
  );
  ...
  const cachedListData = useMemo(() => {
    ...
    return (
      queryClient.getQueryData<Awaited<ReturnType<typeof fetchParcelOrders>>>(queryKey) ?? null
    );
  }, [hasMounted, queryClient, queryKey]);

  const initialSnapshot = useMemo(() => {
    if (!hasMounted) return null;
    return loadClientSnapshot<Awaited<ReturnType<typeof fetchParcelOrders>>>(
      snapshotKey,
      SNAPSHOT_TTL_MS
    );
  ...
  const { data, isLoading, isError, error, isFetching, refetch } = useQuery({
    queryKey,
    queryFn: ({ signal }) => fetchParcelOrders(filtersForQuery, signal),
    enabled: hasMounted,
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          {hasActiveSearch ? (
            ...
            <span className="ml-auto text-[11px] text-gray-500">
              Use header search — Order Id, Receiver, Rider…
            </span>
          )}
        </div>
      </div>
      ...
      <div className="flex items-center justify-between p-2" style={{ backgroundColor: CONTENT_BG }}>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-xs font-medium" style={{ color: DARK_TEXT }}>
            {activeTab.label} — {total.toLocaleString("en-IN")} parcel
            {total === 1 ? "" : "s"}
          </span>
        </div>
            ...
            {showTableLoading ? (
              ...
              hasActiveSearch || hasDateFilter ? (
                <tr>
                  ...
                  >
                    {hasActiveSearch
                      ? `No parcels found for ${urlSearchType}: "${urlSearch}"`
                      : "No parcel orders found for these dates."}
                  </td>
                </tr>
              ) : (
                <tr>
              ...
              orders.map((order) => {
                ...
                const receiver = formatReceiver(order);
                ...
                return (
                  <tr key={order.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-2 py-1.5" style={{ color: TABLE_TEXT }}>
                      <div className="flex items-center gap-1.5">
                        ...
                        <div>
                          ...
                          {receiver.phone ? (
                            <div className="text-[10px] text-gray-500">{receiver.phone}</div>
                          ) : null}
                        </div>
                      </div>
                    </td>
                    ...
                    <td
                      ...
                      style={{ color: TABLE_TEXT }}
                      title={order.pickupAddress ?? undefined}
                    >
                    ...
                    <td
                      className="px-2 py-1.5 max-w-[160px] truncate"
                      style={{ color: TABLE_TEXT }}
                      title={order.dropAddress ?? undefined}
                    >
                    ...
                    <td
                      className="px-2 py-1.5 whitespace-nowrap font-semibold"
                      style={{ color: TABLE_TEXT }}
                    >
                      {formatFare(order.fare)}
                      {order.isCod ? (
                        <div className="text-[10px] font-normal text-amber-700">COD</div>
                      ) : null}
                    </td>
