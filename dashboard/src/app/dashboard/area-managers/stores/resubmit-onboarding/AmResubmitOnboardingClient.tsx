...
import {
  STORE_VERIFICATION_STEP_LABELS,
...
function stepLabel(n: number, fallback?: string | null): string {
  const fromDb = typeof fallback === "string" ? fallback.trim() : "";
  if (fromDb) return fromDb;
  return STORE_VERIFICATION_STEP_LABELS[n] ?? storeVerificationStepLabel(n);
}
...
const AM_RESUBMIT_EXIT_FALLBACK = "/dashboard/area-managers/stores/onboarding-failed";
...
function AmResubmitOnboardingInner() {
  ...
  const exitHref = safeReturnPath(searchParams.get("returnTo"), AM_RESUBMIT_EXIT_FALLBACK);
  ...
  const load = useCallback(async (opts?: { soft?: boolean }) => {
    ...
    try {
      const progressQs = new URLSearchParams({
        storeInternalId: String(storeInternalId),
      });
      if (parentIdParam) progressQs.set("parentId", parentIdParam);

      const [progressRes, rejRes, pendingRes] = await Promise.all([
        fetch(`/api/area-manager/child-store-progress?${progressQs.toString()}`, {
          credentials: "include",
        }),
        fetch(
          `/api/area-manager/store-verification-rejections?store_id=${encodeURIComponent(
            String(storeInternalId)
          )}`,
          { credentials: "include" }
        ),
        fetch(
          `/api/area-manager/onboarding-resubmissions?store_id=${encodeURIComponent(
            String(storeInternalId)
          )}`,
          { credentials: "include" }
        ),
      ...
      if (!progressRes.ok || !progress?.success) {
        if (!soft) toast.error(progress?.error || "Could not load store details");
        return;
      }
      ...
      const form = progress.progress?.form_data || {};
      const storeSnap =
      ...
      const docsSnap =
        storeSnap.documents && typeof storeSnap.documents === "object"
          ...
          : {};
      const step4Form =
        form.step4 && typeof form.step4 === "object"
          ? (form.step4 as Record<string, unknown>)
          : {};
      const s4: Record<string, unknown> = {
        ...step4Form,
        pan_number: pickNonEmpty(lastOld.pan_number, step4Form.pan_number, docsSnap.pan_number),
        pan_image_url: pickNonEmpty(
          lastOld.pan_image_url,
          step4Form.pan_image_url,
          docsSnap.pan_image_url
        ),
        aadhar_number: pickNonEmpty(
          lastOld.aadhar_number,
          step4Form.aadhar_number,
          docsSnap.aadhar_number
        ),
        aadhar_front_url: pickNonEmpty(
          lastOld.aadhar_front_url,
          step4Form.aadhar_front_url,
          docsSnap.aadhar_front_url
        ),
        fssai_number: pickNonEmpty(lastOld.fssai_number, step4Form.fssai_number, docsSnap.fssai_number),
        fssai_expiry_date: pickNonEmpty(
          lastOld.fssai_expiry_date,
          step4Form.fssai_expiry_date,
          docsSnap.fssai_expiry_date
        ),
        fssai_image_url: pickNonEmpty(
          lastOld.fssai_image_url,
          step4Form.fssai_image_url,
          docsSnap.fssai_image_url
        ),
        gst_number: pickNonEmpty(lastOld.gst_number, step4Form.gst_number, docsSnap.gst_number),
        gst_image_url: pickNonEmpty(
          lastOld.gst_image_url,
          step4Form.gst_image_url,
      ...
      const openRejRaw = Array.isArray(rejData.rejections)
        ? // Active rejection until admin verifies — multi-time resubmit allowed after merchant_resubmitted_at.
          rejData.rejections
        : [];
      const steps: OpenRejectionStep[] = openRejRaw
        .map(
          (r: {
            ...
            return {
              step_number: n,
              step_label: stepLabel(n, r.step_label),
      ...
      const step2 =
        form.step2 && typeof form.step2 === "object"
          ? (form.step2 as Record<string, unknown>)
          : {};
      ...
      if (soft) {
        ...
        let nextActiveIdx =
          currentStepNum != null
            ...
            : -1;
        // If this step was completed elsewhere (e.g. partnersite), move to next open step.
      ...
      >;
      const publicId =
        String(rejData.store?.store_id || form.step_store?.storePublicId || "").trim();
      setStorePublicId(publicId);
      setStoreName(
        pickNonEmpty(
          lastOld.store_name,
          s1.store_name,
          storeSnap.store_name,
          form.step_store?.store_name,
          publicId
        ) || "Store"
      );
      const phonesFromS1 = Array.isArray(s1.store_phones)
      ...
      const fromStore = rejData.store?.parent_id != null ? Number(rejData.store.parent_id) : null;
      const fromParam = parentIdParam ? Number(parentIdParam) : null;
      const pk =
        fromStore != null && Number.isFinite(fromStore)
          ? fromStore
          : fromParam != null && Number.isFinite(fromParam)
            ? fromParam
            : null;
      setParentPk(pk);
      ...
      const rejectedBannerRaw = pickNonEmpty(lastOld.banner_r2_key, lastOld.banner_url);
      // After re-reject, Rejected = last resubmitted banner only (not live store image).
  ...
  }, [storeInternalId, parentIdParam, verificationFixStep]);
  ...
  useStoreVerificationLiveSync(
    !loading && !submitting && Number.isFinite(storeInternalId) && storeInternalId > 0
      ? storeInternalId
      : null,
    softRefresh
  );

  const uploadFile = async (file: File, kind: "document" | "bank" | "banner", filename: string) => {
    const form = new FormData();
    form.append("file", file);
    form.append("storeId", String(storeInternalId));
    form.append("kind", kind);
    form.append("filename", filename || file.name);
    const res = await fetch("/api/area-manager/resubmit-upload", {
      method: "POST",
      body: form,
      credentials: "include",
    });
    const data = await res.json().catch(() => ({}));
    const raw = String((data?.proxyUrl || data?.url || data?.key || data?.path || "") as string).trim();
    if (!res.ok || !data?.success || !raw) throw new Error(data?.error || "Upload failed");
  ...
  const buildItemsForStep = async (step: OpenRejectionStep): Promise<StagedItem[] | null> => {
    if (!parentPk || !Number.isFinite(parentPk)) {
    ...
    if (step.step_number === 4 || step.step_number === 6) {
      ...
      for (const item of docsToSubmit) {
        ...
        let proxy = url;
        if (file) {
          const kind = item.key === "bank_proof" ? "bank" : "document";
          const baseName =
            item.key === "bank_proof"
              ? `bank_proof`
              : item.fileField;
          proxy = await uploadFile(file, kind, `${baseName}${file.name.match(/\.[^.]+$/)?.[0] || ""}`);
        }
    ...
    if (needsBannerFix(step.rejection_reason, fields)) {
      ...
      let proxy = bannerUrl;
      if (bannerFile) {
        proxy = await uploadFile(
          bannerFile,
          "banner",
          `banner${bannerFile.name.match(/\.[^.]+$/)?.[0] || ".jpg"}`
        );
      }
      items.push({
        verificationStep: step.step_number,
    ...
    if (items.length === 0) {
      items.push({
        ...
        payload: {
          note: "Area manager fixed this step",
  ...
  const postStaged = async (
    ...
    const res = await fetch("/api/area-manager/onboarding-resubmissions", {
      method: "POST",
      credentials: "include",
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (isLast) {
        // Prevent soft-sync / all-done effect from double-toasting while we finalize.
  ...
  // If AM or Partner finished every rejected step, toast + leave this page.
  useEffect(() => {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#f7f5fb] relative overflow-hidden">
      <div
      ...
      <main className="relative z-10 flex-1 min-h-0 mx-auto flex w-full max-w-5xl gap-8 px-4 sm:px-8 pt-4">
        ...
        <div className="min-w-0 flex-1 min-h-0 overflow-y-auto overscroll-contain pb-16 pr-1">
          ...
          <p className="mt-1.5 text-sm text-slate-600 max-w-xl">
            Help us verify {storeName || storePublicId || "this store"}. Complete one step at a time
            {openSteps.length > 1 ? ` (${activeStepIndex + 1} of ${openSteps.length})` : ""}.
          </p>
              ...
              <div className="px-5 py-5 space-y-4">
                ...
                {activeStepAlreadySubmitted ? (
                  ...
                  <DynamicRejectedFields
                    ...
                    onPreviewUrl={(url, title) => void openPreview(url, title)}
...
export function AmResubmitOnboardingClient() {
  return (
    <Suspense
      fallback={
        <div className="flex h-full min-h-[50vh] items-center justify-center bg-[#f7f5fb]">
          <Loader2 className="h-8 w-8 animate-spin text-violet-600" />
        </div>
      }
    >
      <AmResubmitOnboardingInner />
    </Suspense>
  );
}
