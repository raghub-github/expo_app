...
/** Campaign window from valid_from / valid_till (local calendar days). */
...
/** Human label for which product surface created/updated the offer. */
...
export function offerWasUpdated(offer: Offer): boolean {
  ...
  if (offer.updated_source_platform && offer.updated_source_platform !== offer.created_source_platform) {
    return true;
  }
