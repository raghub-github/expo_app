...
export default function LogsFailuresPage() {
  const { data, isLoading, mutate } = useSWR<{ items: LogRow[] }>(
    "/api/super-admin/notifications/dispatch-logs?status=failed&limit=200",
    fetcher,
  ...
  async function retryRow(r: LogRow) {
    ...
    try {
      ...
      await fetch(`/api/super-admin/notifications/dispatch-logs/${encodeURIComponent(id)}/retry`, {
        method: "POST",
      });
      await mutate();
    } finally {
