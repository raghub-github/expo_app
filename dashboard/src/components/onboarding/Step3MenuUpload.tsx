      ...
      <div className="w-full max-w-6xl h-full overflow-y-auto rounded-2xl bg-white shadow-sm border border-slate-200 p-3 sm:p-6 hide-scrollbar">
            ...
            <div className="flex items-start gap-2 sm:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
                  One type only: up to 5 images, or 1 PDF, or 1 CSV/Excel. Manual entry after verification.
                </p>
              </div>
            </div>
        ...
        {menuUploadMode === "IMAGE" && (
          <div
            ...
            className={`rounded-xl border-2 border-dashed p-4 sm:p-6 transition ${
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-indigo-600 text-white text-sm font-medium hover:bg-indigo-700 min-h-[40px] touch-manipulation"
              >
                Upload images
              </button>
            </div>
        ...
        {menuUploadMode === "PDF" && (
          <div
            ...
            className={`rounded-xl border-2 border-dashed p-4 sm:p-6 transition ${
              isPdfDragActive ? "border-indigo-500 bg-indigo-50" : "border-slate-300 bg-slate-50"
            }`}
          >
            <input
              ...
              className="hidden"
              onChange={(e) => onMenuPdfUpload((e.target.files || [])[0] || null)}
            />
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        {menuUploadMode === "CSV" && (
          ...
          >
            <input
              ...
              accept=".csv,.xls,.xlsx"
              className="hidden"
              onChange={(e) => onMenuSpreadsheetUpload((e.target.files || [])[0] || null)}
            />
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            ...
            <>
              ...
              <ul className="list-none p-0 m-0 flex gap-3 sm:gap-4 overflow-x-auto overflow-y-hidden pr-1">
                {menuImageFiles.map((file, idx) => (
                  <li
                    key={`pending-${idx}`}
                    className="flex flex-col items-stretch gap-2 rounded-lg border border-slate-200 bg-white p-2.5 sm:p-3 shrink-0 w-40 sm:w-44"
                  >
                ...
                {menuUploadedImageUrls.map((url, idx) => (
                  <li
                    key={`uploaded-${menuUploadIds[idx] ?? idx}`}
                    className="flex flex-col items-stretch gap-2 rounded-lg border border-slate-200 bg-white p-2.5 sm:p-3 shrink-0 w-40 sm:w-44"
                  >
                    <div className="w-full aspect-[4/5] rounded-lg bg-slate-100 overflow-hidden flex items-center justify-center">
                      {url ? (
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-xs sm:text-sm font-medium text-slate-800 truncate">
                        {menuUploadedImageNames[idx] || `Image ${idx + 1}`}
                      </p>
                      {url && (
                        <a
                          href={url}
                          ...
                          className="text-[11px] sm:text-xs text-indigo-600 hover:underline"
                        >
                          View
                        </a>
                      )}
            ...
            <>
              ...
              <div className="flex items-center gap-3 rounded-lg border border-slate-200 bg-white p-2.5 sm:p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {menuPdfFile && (
                    <p className="text-xs text-slate-500">{(menuPdfFile.size / (1024 * 1024)).toFixed(2)} MB</p>
                  )}
                  {menuUploadedPdfUrl && (
                    <a
