...
function ServicePointsMapInner({ className = "" }: ServicePointsMapProps) {
  ...
  const { data: servicePoints = [], isLoading, error, refetch, isFetching } = useGetServicePointsQuery();
  ...
  useEffect(() => {
    ...
    fetch('http://127.0.0.1:7242/ingest/2cc0b640-978a-4fbb-81f9-cf64378f704f',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({location:'ServicePointsMap.tsx:32',message:'ServicePoints query state',data:{isLoading,isFetching,hasData:!!servicePoints.length,timeSinceMount:queryCheckTime - componentMountTime},timestamp:Date.now(),sessionId:'debug-session',runId:'run1',hypothesisId:'B'})}).catch(()=>{});
