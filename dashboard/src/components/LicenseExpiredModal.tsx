...
export function LicenseExpiredModal({
  ...
  const fetchStatus = useCallback(async (): Promise<{
    ...
    try {
      const res = await fetch(
        `/api/merchant/stores/${encodeURIComponent(storeId)}/store-documents/status`,
        { credentials: 'include' }
      );
  ...
  const fetchHistoryForPrefix = useCallback(
    async (prefix: MerchantDocumentPrefix) => {
      ...
      try {
        const res = await fetch(
          `/api/merchant/stores/${encodeURIComponent(storeId)}/store-documents/history?licenceType=${encodeURIComponent(prefix)}`,
          { credentials: 'include' }
        );
        const data = await res.json().catch(() => ({}));
        if (res.ok) {
  ...
  const uploadFile = async (
    ...
    const res = await fetch(`/api/merchant/stores/${encodeURIComponent(storeId)}/store-documents/upload`, {
      method: 'POST',
      body: fd,
      credentials: 'include',
    });
