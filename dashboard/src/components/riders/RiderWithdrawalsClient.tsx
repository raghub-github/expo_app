      ...
      {rider && (
        <>
          <CollapsibleTableFilters
            label="Filters"
            activeCount={[status, from, to].filter((v) => v && v !== "all").length}
          ...
          <div className="overflow-hidden relative">
            {loading && withdrawals.length === 0 ? (
              ...
              <>
                ...
                <div className={`overflow-x-auto transition-opacity duration-200 ${loading && withdrawals.length > 0 ? "opacity-70 pointer-events-none" : ""}`}>
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wide">Bank</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wide">Requested</th>
                      <th className="px-4 py-2 text-left text-xs font-medium text-gray-700 uppercase tracking-wide">Processed</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-200">
                    {withdrawals.length === 0 ? (
                      <tr><td colSpan={6} className="px-4 py-8 text-center text-gray-600 text-sm">No withdrawals found.</td></tr>
                    ) : (
                      withdrawals.map((w) => (
                        <tr key={w.id}>
                          <td className="px-4 py-2 text-sm font-mono text-gray-900">{w.id}</td>
                          <td className="px-4 py-2 text-sm text-right font-medium text-gray-900">₹{Number(w.amount).toFixed(2)}</td>
                          <td className="px-4 py-2 text-sm text-gray-900">{w.status}</td>
                          <td className="px-4 py-2 text-sm text-gray-800">{w.bankAcc}</td>
                          <td className="px-4 py-2 text-sm text-gray-800">{new Date(w.createdAt).toLocaleString()}</td>
                          <td className="px-4 py-2 text-sm text-gray-800">{w.processedAt ? new Date(w.processedAt).toLocaleString() : "—"}</td>
                        </tr>
                      ))
                    )}
                  </tbody>
