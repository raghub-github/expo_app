...
import {
  fetchParcelCustomerPricing,
  getParcelCustomerPricingCache,
  invalidateParcelCustomerPricingCache,
  parcelCustomerPricingCacheKey,
  type ParcelCustomerPricingPayload,
} from "@/lib/geo/parcelCustomerPricingCache";
import { SLAB_INPUT_CLS, SlabNumericInput } from "./SlabNumericInput";

export function ParcelCustomerPricingPanel(props: {
  level: string;
  refId: string;
  ...
  const applyPricingPayload = useCallback((payload: ParcelCustomerPricingPayload) => {
  ...
  const refresh = useCallback(
    async (opts?: { force?: boolean; showLoading?: boolean }) => {
      const cacheKey = parcelCustomerPricingCacheKey({
        level: props.level,
        refId: props.refId,
        vehicleType: props.vehicleType,
      });
      const cached = !opts?.force ? getParcelCustomerPricingCache(cacheKey) : null;
      ...
      try {
        const payload = await fetchParcelCustomerPricing({
          level: props.level,
          refId: props.refId,
          vehicleType: props.vehicleType,
          force: opts?.force,
        ...
        toast.error(e instanceof Error ? e.message : "Failed to load parcel pricing");
      } finally {
  ...
  const persistRow = async (
    ...
    const res =
      normalized.id > 0
        ? await fetch(`/api/super-admin/geo/parcel-customer-pricing/${normalized.id}`, {
            method: "PATCH",
            cache: "no-store",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(patchBody),
          })
        : await fetch(`/api/super-admin/geo/parcel-customer-pricing`, {
            method: "POST",
            cache: "no-store",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const del = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/super-admin/geo/parcel-customer-pricing/${id}`, {
        method: "DELETE",
        cache: "no-store",
      });
    ...
    <div className="mt-5 space-y-4">
      ...
      <div className="rounded-xl border border-teal-200 bg-gradient-to-r from-teal-50/80 to-emerald-50/50 px-5 py-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-wide text-teal-800">Parcel fare preview</p>
        ...
        {previewBreakdown?.mode === "customer" ? (
          ...
          <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50/80 px-3 py-2 text-xs text-amber-900">
            Add active slabs to preview parcel customer fare.
          </p>
        )}
      </div>
            ...
            {slabs.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-4 py-8 text-center text-sm text-slate-500">
                  No parcel customer slabs for this vehicle at this node. Click &quot;Add slab&quot; to configure.
                </td>
              </tr>
            ) : (
              slabs.map((s) => {
