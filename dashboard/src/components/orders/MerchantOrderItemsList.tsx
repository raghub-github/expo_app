...
export function MerchantOrderItemsList({
  ...
  const more = maxItems != null && items.length > maxItems ? items.length - maxItems : 0;

  return (
    <div className={className}>
      {requiresUtensils && utensilsLabel ? (
          ...
          <div>
            ...
            {!compact ? (
              <p className="mt-0.5 text-xs text-emerald-800/80">
                We care for your convenience and hygiene.
              </p>
            ) : null}
          </div>
        </div>
        ...
        {preview.length === 0 ? (
          ...
          preview.map((item, i) => {
            ...
            return (
              <div
                key={`${item.name}-${i}`}
                className={`px-3 py-3 ${i < preview.length - 1 ? 'border-b border-gray-100' : ''}`}
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 flex-1 items-start gap-2">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      {custRows.length > 0 ? (
                        <div className="mt-2 ml-1 border-l-2 border-teal-300 pl-2.5">
                          <p className="mb-1 text-[11px] font-bold uppercase tracking-wide text-teal-700">
                            Customizations
                          </p>
