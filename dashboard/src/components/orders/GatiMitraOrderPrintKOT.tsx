...
function auditKotPrint(order: OrdersFoodRow): void {
  ...
  const storeId = order.merchant_store_id;
  if (storeId == null || !Number.isFinite(Number(storeId))) return;
  // Control dashboard merchant portal — same backend audit path as partnersite when available.
  void fetch(`/api/merchant/stores/${storeId}/orders/${orderId}/kot-print`, {
    method: 'POST',
    ...
    body: JSON.stringify({
      ...
      printed_by: 'control_dashboard_merchant',
