...
import { normalizeOrderItems } from "@/lib/orderLineItems";
import {
  historyBadgeClass,
  historyStatusLabel,
} from "@/app/dashboard/merchants/stores/[id]/orders/orders-page-ui";
...
export type UserInsightsOrderDetailsSidesheetProps = {
  open: boolean;
  onClose: () => void;
  /** Dashboard: numeric internal store id from route */
  storeId: string;
...
export function UserInsightsOrderDetailsSidesheet({
  ...
  useEffect(() => {
    ...
    void fetch(`/api/merchant/stores/${storeId}/orders?${params.toString()}`, {
      credentials: "include",
    })
      .then(async (res) => {
  ...
  useEffect(() => {
    if (!open || !order || order.core_only) {
      setOtp(null);
      return;
    }
    const foodRowId = order.orders_food_row_id ?? order.id;
    if (!foodRowId) return;
    void fetch(`/api/merchant/stores/${storeId}/orders/${foodRowId}/otp`, {
      credentials: "include",
    })
      .then((r) => r.json())
      .then((data) => {
          ...
          {loading ? (
            ...
            <>
              ...
              <section className="rounded-xl border border-gray-200 bg-gray-50/50 p-4">
                ...
                {(order as { customer_order_count?: number | null })
                  .customer_order_count != null ? (
                  <DetailRow
                    label="Orders with you"
                    value={String(
                      (order as { customer_order_count?: number })
                        .customer_order_count,
                    )}
                  />
                ) : null}
                {(order as { customer_platform_order_count?: number | null })
                  .customer_platform_order_count != null ? (
                  <DetailRow
                    label="Orders on GatiMitra"
                    value={String(
                      (order as { customer_platform_order_count?: number })
