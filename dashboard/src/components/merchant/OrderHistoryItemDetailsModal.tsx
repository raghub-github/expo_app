...
export type OrderHistoryItemDetailsModalProps = {
  ...
  /** Numeric internal merchant_stores.id from dashboard route */
  storeId: string;
};
...
export function OrderHistoryItemDetailsModal({
    ...
    void (async () => {
      try {
        const res = await fetch(
          `/api/merchant/stores/${storeId}/order-line-item-menu?menuItemId=${menuItemId}`,
          { credentials: "include" },
        );
  ...
  const toggleStock = useCallback(async () => {
    if (!storeId || menu?.id == null) {
      toast.message("Stock", {
        description: "This item is not linked to your menu.",
      });
      return;
    }
    ...
    try {
      const res = await fetch(
        `/api/merchant/stores/${storeId}/menu/items/${menu.id}/stock`,
        {
          method: "PATCH",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ in_stock: !nextOut }),
        },
      );
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data.success) {
        toast.error(
          (data as { error?: string }).error || "Could not update stock",
        );
        return;
      }
  ...
  }, [storeId, menu?.id, outOfStock]);

  if (!open || !lineItem || typeof document === "undefined") return null;

  return createPortal(
    <div
      className="fixed inset-0 z-[2600] flex items-start justify-center overflow-y-auto hide-scrollbar px-4 pb-6 pt-20 sm:pt-24"
      role="presentation"
    >
      <button
        type="button"
        ...
        <div className="overflow-y-auto min-h-0 flex-1 hide-scrollbar">
          ...
          <div className="px-5 py-4">
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-2 min-w-0 flex-1">
                ...
                <div className="min-w-0">
                  ...
                  {lineItem.variantName && <p className="text-xs text-gray-500 mt-0.5">Variant: {lineItem.variantName}</p>}
                  {(lineItem.categoryName || menu?.category_name) && (
                    <p className="text-xs text-gray-500 mt-0.5">{lineItem.categoryName || menu?.category_name}</p>
                  )}
                </div>
              </div>
              <span className="text-base font-semibold text-gray-900 shrink-0 tabular-nums">₹{displayPrice.toFixed(0)}</span>
            </div>
            <p className={`mt-3 text-sm ${description ? "text-gray-600" : "text-rose-400"}`}>{description || "No description added"}</p>
            {lineItem.customizations && lineItem.customizations.length > 0 && (
              <div className="mt-4">
                <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Add-ons / customizations</p>
            ...
            <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm border-t border-gray-100 pt-4">
              <div><dt className="text-gray-500 text-xs">Quantity</dt><dd className="font-medium text-gray-900">{lineItem.quantity}</dd></div>
              <div><dt className="text-gray-500 text-xs">Line total</dt><dd className="font-medium text-gray-900 tabular-nums">₹{lineItem.total.toFixed(2)}</dd></div>
              {menu?.preparation_time_minutes != null && (
                <div><dt className="text-gray-500 text-xs">Prep time</dt><dd className="font-medium text-gray-900">{menu.preparation_time_minutes} min</dd></div>
              )}
              {menu?.serves != null && (
                <div><dt className="text-gray-500 text-xs">Serves</dt><dd className="font-medium text-gray-900">{menu.serves}</dd></div>
              )}
              {menu?.spice_level && (
                <div><dt className="text-gray-500 text-xs">Spice</dt><dd className="font-medium text-gray-900 capitalize">{menu.spice_level}</dd></div>
              )}
            </dl>
          </div>
        </div>
        {menu?.id != null && (
          <div className="px-5 py-4 border-t border-gray-100 flex items-center justify-between gap-3 shrink-0 bg-white">
            ...
            <button
              ...
              onClick={toggleStock}
              className={`relative inline-flex h-7 w-12 shrink-0 rounded-full transition-colors ${outOfStock ? "bg-gray-800" : "bg-gray-300"} ${stockSaving ? "opacity-60" : ""}`}
            >
              <span className={`inline-block h-5 w-5 transform rounded-full bg-white shadow transition-transform mt-1 ${outOfStock ? "translate-x-6 ml-0.5" : "translate-x-1"}`} />
            </button>
          </div>
        )}
      </div>
