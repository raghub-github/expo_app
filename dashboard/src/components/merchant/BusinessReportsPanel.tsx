...
export function BusinessReportsPanel({ storeInternalId, periodPreset = "this_week", subview }: Props) {
  ...
  React.useEffect(() => {
    ...
    setError(null);
    void fetch(
      `/api/merchant/stores/${storeInternalId}/growth/business-insights?period=${encodeURIComponent(period)}`,
      { cache: "no-store" }
    )
      .then(async (res) => {
        if (!res.ok) {
