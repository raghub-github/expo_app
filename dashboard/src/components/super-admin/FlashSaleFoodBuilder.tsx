...
const controlCls =
  "w-full min-h-[38px] rounded-lg border border-slate-200/90 bg-white px-3 py-2 text-sm text-slate-900 placeholder:text-slate-400 focus:border-indigo-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/20";
            ...
            >
              ...
              <div className="border-b border-slate-100 px-4 py-3">
                <input
                  ...
                  autoFocus
                />
                <label className="mt-2.5 flex items-center gap-2 text-xs text-slate-600">
                  <input
                    type="checkbox"
              ...
              <div className="min-h-0 flex-1 overflow-y-auto">
                {loadingMenu ? (
                  ...
                  <div className="space-y-3 px-4 py-5">
                    ...
                    <button
                      type="button"
                      className="rounded-lg bg-indigo-600 px-3.5 py-2 text-sm font-medium text-white"
                      onClick={() => {
    ...
    <div className="space-y-3">
      <div className="relative space-y-1.5">
        <label className="block text-[11px] font-medium text-slate-500">
          Add outlets (search &amp; select multiple)
        </label>
        <input
          className={controlCls}
          value={storeQuery}
          ...
          onBlur={() => {
            // Delay so a click on a result can register before the list unmounts.
        ...
        />
        {storeSearchOpen ? (
          <div className="absolute left-0 right-0 z-30 mt-1 max-h-40 overflow-y-auto rounded-xl border border-slate-200 bg-white shadow-lg">
            {loadingStores ? (
        ...
        <div className="space-y-2">
          {stores.map((s) => {
            ...
            return (
              <div
                key={s.id}
                className="flex flex-wrap items-center gap-2 rounded-xl border border-indigo-100 bg-indigo-50/40 px-3 py-2.5"
              >
      ...
      {items.length > 1 ? (
        <div className="flex flex-wrap items-end gap-2 rounded-xl border border-slate-200 bg-slate-50/60 p-2.5">
          ...
          <button
            type="button"
            className="min-h-[38px] rounded-lg bg-slate-900 px-3.5 text-sm font-medium text-white"
            onClick={applyBulkPrice}
          >
            Apply to all
          </button>
          {bulkError ? <p className="w-full text-[11px] text-red-600">{bulkError}</p> : null}
        </div>
      ) : null}

      {items.length > 0 ? (
        <div className="overflow-hidden rounded-xl border border-slate-200">
          <div className="grid grid-cols-[1fr_auto] gap-2 bg-slate-50 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wide text-slate-500 sm:grid-cols-[minmax(0,1.3fr)_64px_100px_72px_72px_72px]">
            <span>Item</span>
            <span className="hidden sm:block">Original</span>
            <span className="hidden sm:block">Flash price</span>
            <span className="hidden text-right sm:block">% Off</span>
            <span className="hidden text-right sm:block">Platform</span>
          ...
          {stores.map((s) => {
            ...
            return (
              <div key={`group-${s.id}`}>
                <div className="border-t border-slate-100 bg-slate-50/80 px-3 py-1.5 text-[11px] font-semibold text-slate-600">
                  {s.name}
                ...
                {storeItems.map((it) => {
                  ...
                  return (
                    <div
                      key={k}
                      className="grid grid-cols-1 gap-2 border-t border-slate-100 px-3 py-2.5 sm:grid-cols-[minmax(0,1.3fr)_64px_100px_72px_72px_72px] sm:items-center"
                    >
                      <div className="min-w-0">
                        <p className="truncate text-sm font-medium text-slate-900">{it.name}</p>
                        <p className="text-[11px] text-slate-500 sm:hidden">
                          Original ₹{Math.round(it.originalCustomerPrice)}
                          {livePct != null ? ` · ${livePct}% off` : ""}
                          {subsidy > 0 ? ` · platform ₹${subsidy.toFixed(0)}` : ""}
                        </p>
                      </div>
                      <p className="hidden text-sm text-slate-400 line-through sm:block">
                        ₹{Math.round(it.originalCustomerPrice)}
                      </p>
                      <div>
                        <input
                          className={cn(controlCls, msg && msg !== "saved" && "border-red-400")}
                          inputMode="decimal"
                          value={draft}
                          onChange={(e) => {
                      ...
                      <div className="hidden justify-end sm:flex">
                        {livePct != null ? (
                          <span className="inline-flex rounded-full bg-orange-50 px-2 py-0.5 text-[11px] font-bold tabular-nums text-orange-700">
                            {livePct}% off
                          </span>
                        ) : (
                      ...
                      <p className="hidden text-right text-sm tabular-nums text-slate-700 sm:block">
                        {subsidy > 0 ? `₹${subsidy.toFixed(0)}` : "—"}
                      </p>
                      <div className="flex justify-end">
                        <button
                          type="button"
                          className={cn(
                            "inline-flex min-h-[34px] items-center gap-1 rounded-lg px-2.5 text-xs font-semibold",
                            dirty || msg !== "saved"
                        ...
                        >
                          {msg === "saved" && !dirty ? <Check className="h-3.5 w-3.5" /> : null}
        ...
        <p className="text-xs text-slate-500">
          Pick menu items per outlet, then set Flash prices. % off updates live.
        </p>
      ) : null}

      {items.some((it) => parseFlashInput(it.flashPrice) != null) ? (
        <div className="flex items-center justify-center rounded-xl border border-emerald-100 bg-emerald-50/60 px-3 py-2 text-center text-[11px] leading-relaxed text-slate-600">
          Customer pays the Flash price + existing checkout fees. Merchant CTC is unchanged. Platform
          funds the gap. One campaign can cover multiple outlets and many items.
        </div>
      ) : null}
