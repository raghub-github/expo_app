...
import { shouldBroadcastWeatherUpdate } from "./weather.alerts.js";
import {
...
async function fetchAndStoreZone(args: {
  ...
  if (changed) {
    ...
    if (args.broadcast) {
      ...
      const afterCmp = mappedToComparable(mapped);
      if (shouldBroadcastWeatherUpdate(reasons, afterCmp)) {
...
async function buildContextFromSnapshot(
  ...
  const mapped = providerPayload?.mapped as
    | { isThunderstorm?: boolean; rainMm?: number; precipitationMm?: number }
    | undefined;
  const alertActive = (await import("./weather.alerts.js")).isRainAlertActive({
    rainDetected: snapshot.rainDetected,
    rainIntensityMm: snapshot.rainIntensityMm,
    weatherSeverity: snapshot.weatherSeverity,
    isThunderstorm: mapped?.isThunderstorm,
    thresholds,
  });
  const severity = alertActive ? snapshot.weatherSeverity : "CLEAR";
  const rainDetected = alertActive ? snapshot.rainDetected : false;
  return buildCustomerWeatherPresentation({
    ...
    etaDelayMinutes: alertActive ? etaDelayForSeverity(severity, thresholds) : 0,
...
export async function resolveMerchantZoneWeather(args: {
  ...
  return buildMerchantWeatherPresentation({
    ...
    rainDetected: customerCtx.rainDetected,
    rainIntensityMm: customerCtx.rainIntensityMm,
    alertThresholds: await getWeatherThresholds(),
  });
}
