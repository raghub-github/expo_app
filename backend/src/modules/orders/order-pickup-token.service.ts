...
export async function validatePickupScan(input: PickupScanInput): Promise<PickupScanResult> {
  ...
  return sql.begin(async (tx) => {
    ...
    if (!row) {
      return rejectScan(tx, { device, ...input }, null, "TOKEN_NOT_FOUND", 404);
    }
    if (row.status === "INVALIDATED") {
      return rejectScan(tx, { device, ...input }, row, "TOKEN_INVALIDATED", 409);
    }
    if (isTerminalOrderStatus(row.core_status, row.current_status)) {
      ...
      return rejectScan(tx, { device, ...input }, row, "TOKEN_EXPIRED", 410);
    }
    ...
    if (row.status !== "ACTIVE") {
      return rejectScan(tx, { device, ...input }, row, "TOKEN_NOT_ACTIVE", 409);
    }
    ...
    if (currentRider == null || currentRider !== Number(input.riderId)) {
      return rejectScan(tx, { device, ...input }, row, "WRONG_RIDER", 403);
    }
    if (row.active_assignment_picked_up_at) {
      return rejectScan(tx, { device, ...input }, row, "ALREADY_PICKED_UP", 409);
    }
...
export async function validatePickupByOtp(input: PickupOtpInput): Promise<PickupScanResult> {
  ...
  return sql.begin(async (tx) => {
    ...
    if (row.status === "INVALIDATED") {
      return rejectScan(tx, { token: row.token, device, ...input }, row, "TOKEN_INVALIDATED", 409);
    }
    if (isTerminalOrderStatus(row.core_status, row.current_status)) {
      if (row.status !== "EXPIRED" && row.status !== "INVALIDATED") {
        await tx`
      ...
      return rejectScan(tx, { token: row.token, device, ...input }, row, "TOKEN_EXPIRED", 410);
    }
    if (row.status === "USED" || row.status === "EXPIRED") {
      await tx`
        UPDATE order_pickup_tokens
    ...
    if (row.status !== "ACTIVE") {
      return rejectScan(tx, { token: row.token, device, ...input }, row, "TOKEN_NOT_ACTIVE", 409);
    }
    ...
    const currentRider =
      row.current_rider_id != null && Number.isFinite(Number(row.current_rider_id))
        ...
        : row.assigned_rider_id != null && Number.isFinite(Number(row.assigned_rider_id))
          ...
          : null;
    if (currentRider == null || currentRider !== Number(input.riderId)) {
      return rejectScan(tx, { token: row.token, device, ...input }, row, "WRONG_RIDER", 403);
    }
    if (row.active_assignment_picked_up_at) {
      return rejectScan(tx, { token: row.token, device, ...input }, row, "ALREADY_PICKED_UP", 409);
    }
    if (!row.pickup_otp || String(row.pickup_otp).trim() === "") {
      return rejectScan(tx, { token: row.token, device, ...input }, row, "OTP_UNAVAILABLE", 409);
    }
    if (String(row.pickup_otp).trim() !== otp) {
      return rejectScan(tx, { token: row.token, device, ...input }, row, "OTP_MISMATCH", 401);
    }
