-- =============================================================================
-- 0393_merchant_store_transactional_reset_v1.sql
-- =============================================================================
-- Creates purge_merchant_store_transactional_data() — wipe ALL transactional data
-- for ONE merchant store so it is clean for fresh orders.
--
-- KEEPS: merchant_stores row, menu, hours, bank/UPI accounts, staff, contracts,
--        offers/coupons definitions, wallet shell (zeroed), push tokens, settings.
--
-- DELETES: orders, payments, wallet ledger, payouts, tickets, reviews, complaints,
--          dispatch/rider assignment rows, GM ledger, analytics derived from orders.
--
-- Does NOT delete customers, riders, or global config catalogs.
--
-- Usage (Supabase SQL editor):
--   -- 1) Preview counts (no changes):
--   SELECT public.purge_merchant_store_transactional_data('GMMC1025', NULL, FALSE);
--
--   -- 2) Execute purge (wrap in transaction, review, then COMMIT):
--   BEGIN;
--   SELECT public.purge_merchant_store_transactional_data('GMMC1025', NULL, TRUE);
--   -- ROLLBACK;  -- dry run inside txn
--   COMMIT;
-- =============================================================================
...
COMMENT ON FUNCTION public.purge_merchant_store_transactional_data(TEXT, BIGINT, BOOLEAN) IS
  'Dev/ops: wipe transactional data for one merchant store (orders, wallet ledger, tickets, payouts). Keeps menu, profile, bank accounts. p_execute=FALSE previews only.';
