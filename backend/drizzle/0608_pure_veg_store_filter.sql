-- Pure-veg listing: return is_pure_veg from nearby RPC, and stop treating
-- "has at least one veg item" as a pure-veg restaurant.
--
-- I/O-safe / idempotent:
--   • DROP/CREATE OR REPLACE function only (no table rewrite, no full seq scan
--     of merchant_menu_items as a standalone pass).
--   • Targeted UPDATE of merchant_stores rows that are currently flagged
--     is_pure_veg AND have a visible non-veg item. Unchanged rows are skipped.
--   • No new hot-table indexes. No backfill that sets is_pure_veg = true.
...
AS $$
  WITH base AS (
    SELECT
      ...
      ms.parent_id,
      COALESCE(ms.is_pure_veg, false) AS is_pure_veg,
      (
        6371.0 * acos(
          LEAST(
        ...
        OR (
          ...
          AND NOT EXISTS (
            ...
            WHERE mmi.store_id = ms.id
              AND COALESCE(mmi.is_deleted, FALSE) = FALSE
              AND mmi.approval_status::text IN ('APPROVED', 'PENDING')
              AND LOWER(TRIM(COALESCE(mmi.food_type::text, ''))) NOT IN (
...
COMMENT ON FUNCTION public.get_nearby_merchant_stores(double precision, double precision, double precision, integer, boolean) IS
  'Nearby ACTIVE stores with ≥1 customer-visible menu item. veg_mode keeps declared pure-veg stores or stores whose every visible item is veg. Returns is_pure_veg.';
...
-- Clear the flag only where it is currently true AND a visible item is not veg.
UPDATE public.merchant_stores ms
SET is_pure_veg = FALSE
WHERE ms.is_pure_veg IS TRUE
  AND EXISTS (
    SELECT 1
    FROM public.merchant_menu_items mmi
    WHERE mmi.store_id = ms.id
