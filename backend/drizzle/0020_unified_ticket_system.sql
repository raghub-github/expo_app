...
CREATE INDEX unified_tickets_ticket_id_idx ON unified_tickets(ticket_id);
CREATE INDEX unified_tickets_order_id_idx ON unified_tickets(order_id) WHERE order_id IS NOT NULL;
CREATE INDEX unified_tickets_customer_id_idx ON unified_tickets(customer_id) WHERE customer_id IS NOT NULL;
CREATE INDEX unified_tickets_rider_id_idx ON unified_tickets(rider_id) WHERE rider_id IS NOT NULL;
CREATE INDEX unified_tickets_merchant_store_id_idx ON unified_tickets(merchant_store_id) WHERE merchant_store_id IS NOT NULL;
CREATE INDEX unified_tickets_merchant_parent_id_idx ON unified_tickets(merchant_parent_id) WHERE merchant_parent_id IS NOT NULL;
CREATE INDEX unified_tickets_ticket_type_idx ON unified_tickets(ticket_type);
CREATE INDEX unified_tickets_ticket_source_idx ON unified_tickets(ticket_source);
CREATE INDEX unified_tickets_service_type_idx ON unified_tickets(service_type);
CREATE INDEX unified_tickets_ticket_title_idx ON unified_tickets(ticket_title);
CREATE INDEX unified_tickets_ticket_category_idx ON unified_tickets(ticket_category);
CREATE INDEX unified_tickets_status_idx ON unified_tickets(status);
CREATE INDEX unified_tickets_priority_idx ON unified_tickets(priority);
CREATE INDEX unified_tickets_assigned_to_agent_id_idx ON unified_tickets(assigned_to_agent_id) WHERE assigned_to_agent_id IS NOT NULL;
CREATE INDEX unified_tickets_raised_by_type_idx ON unified_tickets(raised_by_type);
CREATE INDEX unified_tickets_raised_by_id_idx ON unified_tickets(raised_by_id) WHERE raised_by_id IS NOT NULL;
CREATE INDEX unified_tickets_parent_ticket_id_idx ON unified_tickets(parent_ticket_id) WHERE parent_ticket_id IS NOT NULL;
CREATE INDEX unified_tickets_created_at_idx ON unified_tickets(created_at);
CREATE INDEX unified_tickets_updated_at_idx ON unified_tickets(updated_at);
CREATE INDEX unified_tickets_tags_idx ON unified_tickets USING GIN(tags);
CREATE INDEX unified_tickets_metadata_idx ON unified_tickets USING GIN(metadata);
CREATE INDEX unified_tickets_email_thread_id_idx ON unified_tickets(email_thread_id) WHERE email_thread_id IS NOT NULL;

-- Composite Indexes for Common Queries
CREATE INDEX unified_tickets_status_priority_idx ON unified_tickets(status, priority);
CREATE INDEX unified_tickets_service_type_status_idx ON unified_tickets(service_type, status);
CREATE INDEX unified_tickets_ticket_type_service_type_idx ON unified_tickets(ticket_type, service_type);
CREATE INDEX unified_tickets_assigned_agent_status_idx ON unified_tickets(assigned_to_agent_id, status) WHERE assigned_to_agent_id IS NOT NULL;
...
-- Indexes
CREATE INDEX unified_ticket_messages_ticket_id_idx ON unified_ticket_messages(ticket_id);
CREATE INDEX unified_ticket_messages_sender_type_idx ON unified_ticket_messages(sender_type);
CREATE INDEX unified_ticket_messages_sender_id_idx ON unified_ticket_messages(sender_id) WHERE sender_id IS NOT NULL;
CREATE INDEX unified_ticket_messages_created_at_idx ON unified_ticket_messages(created_at);
CREATE INDEX unified_ticket_messages_is_internal_note_idx ON unified_ticket_messages(is_internal_note);
...
-- Indexes
CREATE INDEX unified_ticket_activities_ticket_id_idx ON unified_ticket_activities(ticket_id);
CREATE INDEX unified_ticket_activities_activity_type_idx ON unified_ticket_activities(activity_type);
CREATE INDEX unified_ticket_activities_actor_type_idx ON unified_ticket_activities(actor_type);
CREATE INDEX unified_ticket_activities_created_at_idx ON unified_ticket_activities(created_at);
...
-- Indexes
CREATE INDEX ticket_title_config_ticket_title_idx ON ticket_title_config(ticket_title);
CREATE INDEX ticket_title_config_is_active_idx ON ticket_title_config(is_active);
CREATE INDEX ticket_title_config_display_order_idx ON ticket_title_config(display_order);
...
-- Indexes
CREATE INDEX ticket_auto_generation_rules_trigger_event_idx ON ticket_auto_generation_rules(trigger_event);
CREATE INDEX ticket_auto_generation_rules_is_active_idx ON ticket_auto_generation_rules(is_active);
CREATE INDEX ticket_auto_generation_rules_is_enabled_idx ON ticket_auto_generation_rules(is_enabled);
