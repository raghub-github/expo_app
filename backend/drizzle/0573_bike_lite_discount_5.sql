-- Bike Lite customer fare is parent Bike − admin amount (default ₹5).
-- EV Auto customer option is parent Auto − admin amount (default ₹5).
-- Config rows stay inactive; quote pipeline reads value_numeric only.
...
BEGIN
  ...
  ELSE
    UPDATE billing_pricing_rules
    SET value_numeric = 5, updated_at = NOW()
    WHERE upper(trim(service_type)) = 'RIDE'
      AND charge_subtype = 'RIDE_EV_AUTO_DISCOUNT'
...
-- Customer-facing EV Auto option (Auto − offset), same pattern as Bike Lite.
