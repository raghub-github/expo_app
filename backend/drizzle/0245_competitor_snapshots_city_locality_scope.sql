-- City vs pincode (locality) competitor scopes + refresh function update.
-- period_key: 90d_city | 90d_locality

UPDATE public.merchant_store_competitor_snapshots
SET period_key = '90d_city'
WHERE period_key = '90d';
...
COMMENT ON FUNCTION public.refresh_merchant_store_competitor_snapshots(BIGINT, TEXT) IS
  'Recompute competitor snapshots: p_match_scope = city (same city) or locality (same pincode).';

-- Backfill both scopes for all stores (may take a while on large DBs).
SELECT public.refresh_merchant_store_competitor_snapshots(NULL, 'city');
SELECT public.refresh_merchant_store_competitor_snapshots(NULL, 'locality');
