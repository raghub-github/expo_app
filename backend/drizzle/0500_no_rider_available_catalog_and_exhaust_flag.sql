-- 0500: ensure dispatch strategy config exists (from main 0492), then NO_RIDER + exhaust flag.
...
-- Catalog + exhaust flag (local 0500; main used 0496 for the column only).
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables
    WHERE table_schema = 'public'
      AND table_name = 'platform_rider_dispatch_strategy_config'
  ) THEN
    ...
    COMMENT ON COLUMN public.platform_rider_dispatch_strategy_config.auto_cancel_on_exhaustion IS
      'When true, food order unfilled after max_retry_duration_seconds is auto-cancelled + refunded.';
  END IF;
END $$;
