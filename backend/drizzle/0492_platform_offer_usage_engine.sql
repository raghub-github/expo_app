...
-- Idempotent / production-safe / backward compatible.

-- ---------------------------------------------------------------------------
-- billing_platform_offers — usage + lifecycle config
-- ---------------------------------------------------------------------------
...
COMMENT ON COLUMN public.billing_platform_offers.max_uses_total IS
  'Lifetime platform redemptions across all customers; NULL = unlimited.';
COMMENT ON COLUMN public.billing_platform_offers.max_uses_per_user IS
  'Max successful redemptions per customer; NULL = unlimited.';
COMMENT ON COLUMN public.billing_platform_offers.max_uses_per_day IS
  'Max redemptions per customer per calendar day (UTC); NULL = unlimited.';
COMMENT ON COLUMN public.billing_platform_offers.max_uses_per_month IS
  'Max redemptions per customer per calendar month (UTC); NULL = unlimited.';
COMMENT ON COLUMN public.billing_platform_offers.consume_mode IS
  'ON_PLACED = count usage at order placement; ON_DELIVERED = finalize on delivery.';
COMMENT ON COLUMN public.billing_platform_offers.restore_on_cancel IS
  'When true, cancelled/failed orders release reserved/consumed usage.';
COMMENT ON COLUMN public.billing_platform_offers.restore_on_refund IS
  'When true, refunded orders release consumed usage and restore budget.';

-- ---------------------------------------------------------------------------
-- platform_offer_usages — single source of truth for redemptions
-- ---------------------------------------------------------------------------
...
COMMENT ON TABLE public.platform_offer_usages IS
  'Platform offer redemption ledger — enforces per-user/day/month/lifetime limits and budget.';

-- Clamp any pre-existing overspend before CHECK (best-effort, no data loss of rows)
...
-- Budget consistency guards (never negative; never exceed total when total is set)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- Backfill consumed rows from historical platform offer_order_applications (best-effort).
