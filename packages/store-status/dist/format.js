 ...
 * circuit the label.
 */
function formatLocalHHMM(iso, timezone) {
    if (!iso)
        return null;
 ...
 */
function relativeDayLabel(targetIso, timezone, now = new Date()) {
    ...
    if (getYmd(fmt.formatToParts(tomorrow)) === targetYmd)
        return "tomorrow";
...
function isValidPhase(p) {
    return typeof p === "string" && VALID_PHASES.includes(p);
}
/**
 ...
 */
export function formatStoreStatusLabel(input, now = new Date()) {
